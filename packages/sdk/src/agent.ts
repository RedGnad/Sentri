...
function buildMarketPrompt(input: {
  ...
  // When the vault is effectively fully deleveraged (riskBalance = 0 or below the
  // dust threshold), the drawdown is locked in and cannot recover by holding
  // stablecoins. Let market conditions drive re-entry instead;
  // POST_DEFENSIVE_REENTRY guards gate the timing.
  const effectiveDrawdownPct =
    riskN <= 0 || riskValueUsd < MIN_RISK_POSITION_USD ? 0 : drawdownPct;
